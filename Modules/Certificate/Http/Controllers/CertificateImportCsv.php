...
class CertificateImportCsv extends Controller
{
    public function __invoke(Request $request)
    {
        abort_if(Gate::denies('import-csv-certificate'), Response::HTTP_FORBIDDEN, __('permission::messages.gate_denies'));
        try {
            $units = Unit::all();
            $category = Category::all();
            $brand = Brand::all();
            $certificates = $request->all();
            $removed = array_shift($certificates);
            $certificatesArray = [];
            foreach ($certificates as $key => $value) {
                ...
                $certificatesArray[] = [
                    ...
                    'certificate_desc' => isset($value['certificate_desc']) ? trim($value['certificate_desc']) : null,
                    ...
                    'certificate_type' => isset($value['certificate_type']) ? Arr::first(Certificate::PRODUCT_TYPES, function ($valueInArray, $key) use ($value) {
                        return $valueInArray['name'] == trim($value['certificate_type']);
                    })['id'] : 1,
                    'is_draft' => isset($value['is_draft']) && in_array(Str::lower(trim($value['is_draft'])), Controller::ACTIVE_ARRAY) ? true : false,
                ];
            }
            $errors = [];
            foreach ($certificatesArray as $certificate) {
                $validator = Validator::make($certificate, [
                    'name'          => ['required', 'min:3', 'max:100', Rule::unique('certificates', 'name')->whereNull('deleted_at')],
                    'label'         => ['required', 'min:3', 'max:100', 'string', Rule::unique('certificates', 'label')->whereNull('deleted_at')],
                    'certificate_desc'  => ['sometimes', 'max:255', 'nullable', 'string'],
                    'category_id'   => ['sometimes', 'integer', 'nullable'],
                    'brand_id'      => ['sometimes', 'integer', 'nullable'],
                    'barcode'       => ['sometimes', 'max:255', 'nullable', 'string', Rule::requiredIf(!!$certificate['barcode_type'])],
                    'barcode_type'  => ['integer', 'sometimes', 'nullable', Rule::requiredIf(!!$certificate['barcode'])],
                    ...
                    'tax_type'      => ['integer', 'sometimes', 'nullable', Rule::requiredIf(!!$certificate['tax']), 'integer', Rule::in([1, 2])],
                    'tax'           => ['sometimes', Rule::requiredIf(!!$certificate['tax_type']), 'numeric', 'min:0'],
                    'stock_alert'   => ['sometimes', 'nullable', 'integer', 'min:0'],
                    'is_draft'     => ['nullable', 'boolean'],
                    'certificate_type'    => ['nullable', 'integer', Rule::in([1, 2, 3])],
                    'remarks'       => ['string', 'nullable'],
                ]);

                if ($validator->fails()) {
                    $errors[$certificate['name']] = $validator->errors();
                }
            }
            if ($errors) {
                return $this->error(['errors' => $errors], Response::HTTP_UNPROCESSABLE_ENTITY);
            }
            Certificate::upsert($certificatesArray, ['name', 'label']);
